...
// ------------------------------------------------------------------------
    /*
     * Using atomicreference because java does not provide mutable wrapper.
     * Its like
     * double pointer in C.
     */
    private void delete(Node root, int data, AtomicReference<Node> rootReference) 
    {
        ...
        if(root.data == data) 
        {
            //if node to be deleted has 0 or 1 non-null children then we have
    ...
    private void deleteOneChild(Node nodeToBeDelete, AtomicReference<Node> rootReference) 
    {
        ...
        //Determine if child dependency is either the left or right 
        //of its parent. Otherwise, this becomes NULL.
        Node child = nodeToBeDelete.right.isNullLeaf ? nodeToBeDelete.left : nodeToBeDelete.right;

        //Replace node with either (one non-null child if it exists) or (null child).
