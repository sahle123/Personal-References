...
const int VERTICES = 50000;   // Number of points that will be generated
int DRAW_SPACE = 50000;    // This will manage the redraw limit.

int DO_REDRAW = 0;      // 1 = Redraw. 0 = Don't Redraw
...
void key(unsigned char ch, int x, int y)
{
    ...
    else if ((ch == 'e') || (ch == 'E'))
    {
        ...
        /// I call glutTimerFunc 6 times to speed the draw speed up.
        /// This may not be the best way, but this proved to be the
        /// most successful way I could come up with...
        ...
        // This is a toggle for redrawing.
        DO_REDRAW = ((DO_REDRAW + 1) % 2);
    }
...
void timer(int i)
{
    if(DO_REDRAW)
    {
        //printf("Here's the point x: %f\n", LOR_PTS[I][0]);
        //printf("Here's the point the iterator: %d\n", I);

        // Rerun the timer (I use vertices -1 to rectify end cases.)
        if(DRAW_SPACE <= VERTICES-1)
        {
            // Tell GLUT it is necessary to redisplay the scene
            glutPostRedisplay();
            // Increment draw space.
            ++DRAW_SPACE;
            // Do next iteration of the timer function.
            glutTimerFunc(1, timer, 0);
        }
