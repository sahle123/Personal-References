...
*
*   Please note that Prototypes have many Helper functions that I did
*   not specify in there. Most of this code has been taken and modified
*   from Professor William's code.
*
*   Notes:
//  Load an OBJ file
...
#else
#include <GL/glut.h>
#include "glWindow_wrapper.h"
#endif

//  Material Structure
//  To handle .mtl files of .obj files
typedef struct
{
...
unsigned int LoadTexBMP(const char* file)
{
    ...
    if (dx<1 || dx>static_cast<unsigned int>(max)) Fatal("%s image width %d out of range 1-%d\n",file,dx,max);
    if (dy<1 || dy>static_cast<unsigned int>(max)) Fatal("%s image height %d out of range 1-%d\n",file,dy,max);
...
static void LoadMaterial(const char* file)
{
    ...
    while ((line = readline(f)))
    {
        ...
        if ((str = readstr(line,"newmtl")))
        {
            ...
            mtl[k].Ka[0] = mtl[k].Ka[1] = mtl[k].Ka[2] = 0;
            mtl[k].Ka[3] = 1;
            mtl[k].Kd[0] = mtl[k].Kd[1] = mtl[k].Kd[2] = 0;
            mtl[k].Kd[3] = 1;
            mtl[k].Ks[0] = mtl[k].Ks[1] = mtl[k].Ks[2] = 0;
            mtl[k].Ks[3] = 1;
            mtl[k].Ns  = 0;
            mtl[k].d   = 0;
            mtl[k].map = 0;
